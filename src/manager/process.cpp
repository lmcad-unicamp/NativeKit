...
void Process::pause() {
	printf("Processo %lu: waiting\n", id );
}

void Process::start() {
	printf("Process %lu: running\n", id );
}
...
void Process::saveState( Registers &registers ) {
	this->registers.copy( registers );
}

void Process::loadState( Registers &registers ) {
	registers.copy( this->registers );
}
