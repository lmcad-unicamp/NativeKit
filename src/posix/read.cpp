...
void sys_read( System *sys, Registers &registers ) {
	FileSystem *fs;
	registers.r0 = 0;
	...
	if ( registers.r1 == 0 ) {
		InputDevice *in = sys->getDefaultInput();

		if ( in && in->isReady() )
			registers.r0 = in->read( (char*) registers.r2, registers.r3 );
	} else if ((fs = sys->getFileSystem())) {
		registers.r0 = fs->read( registers.r1, (char*) registers.r2, registers.r3 );
	}
}
