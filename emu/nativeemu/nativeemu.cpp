...
static const char* inputfn = "./nes.fmv\0";
...
namespace IO
{
    ...
    void PutPixel(unsigned px,unsigned py, unsigned pixel, int offset)
    {
		...
		//
		int pos = (py * 256 + px) * 3;
		s[pos    ] = (char) ( palette[offset][prev%64][pixel]       );
		s[pos + 1] = (char) ( palette[offset][prev%64][pixel] >> 8  );
		s[pos + 2] = (char) ( palette[offset][prev%64][pixel] >> 16 );
        prev = pixel;
    }
...
int main() {
	...
	system.setDefaultInput( keyboard );
	system.setDefaultOutput( video );
